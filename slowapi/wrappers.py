...
class LimitGroup(object):
    ...
    def __iter__(self) -> Iterator[Limit]:
        if callable(self.__limit_provider):
            if 'key' in inspect.signature(key_func).parameters.keys():
                assert "request" in inspect.signature(key_func).parameters.keys()
                assert self.request
                limit_raw = self.__limit_provider(key_func(self.request))
            else:
